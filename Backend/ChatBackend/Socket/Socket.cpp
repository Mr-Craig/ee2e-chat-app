...
void websocket::init()
{
    ...
    std::string portEnv = utils::misc::getEnvironmentVariable("CHATAPP_PORT");

    if(!portEnv.empty()) {
        portNumber = std::stoull(portEnv);
    }

    app.listen(portNumber, [this](auto* socket) {
        if (socket) {
            LOG("Socket", "Listening on Port {}", portNumber);
        }
        else {
            FATAL("Socket", "Unable to listen on port {}", portNumber);
        }
    });
}
...
{
    std::string socketId = utils::misc::generateUniqueId("sk");
